...
export const sendMessage = async (request: Request, response: Response) => {
  ...
  if (!channelId || !message || !type || !conversationId) {
    response
      .status(500)
      .send({ errors: { message: "Missing value", code: "missing_value" } });
    return;
  }
  // @ts-ignore
  ...
  const senderName = await getUserDisplayName(
    senderId,
    type === COMMUNITY_MESSAGE_TYPE ? channelId : ""
  );
...
const getConversationDetail = async (
  ...
  const mainPath =
    type === COMMUNITY_MESSAGE_TYPE || type === FAULT_REPORT_MESSAGE_TYPE
      ? HOUSING_COMPANIES
      : SUPPORT_CHANNELS;
  try {
    return (
...
export const getConversationRequest = async (
  ...
  try {
    ...
    if (type === COMMUNITY_MESSAGE_TYPE) {
      ...
      if (
        conversation.user_ids?.includes(senderId) !== true &&
        !isAdminRole(senderId)
      ) {
        response
          .status(403)
          .send({
...
export const setConversationSeenRequest = async (
  request: Request,
  response: Response
) => {
  const type = request.body?.type;
  ...
  if (type === COMMUNITY_MESSAGE_TYPE || type === FAULT_REPORT_MESSAGE_TYPE) {
    if (!(await isCompanyTenant(userId, channelId))) {
      response
        .status(403)
        .send({ errors: { message: "Unauthorized", code: "unauthorized" } });
      return;
    }
    await admin
      .firestore()
    ...
    response
      .status(500)
      .send({ errors: { message: "Invalid type", code: "invalid_type" } });
    return;
  }
  const conversation = await getConversationDetail(
    type,
...
export const joinConversationRequest = async (
  request: Request,
  response: Response
) => {
  const type = request.body.type;
...
export const startNewConversationRequest = async (
  request: Request,
  response: Response
) => {
  const type = request.body?.type;
  ...
  if (type === COMMUNITY_MESSAGE_TYPE) {
    ...
    const activeSubscription = await hasOneActiveSubscription(
      channelIdOrCompanyId
    );
    ...
    const conversationCount = await getCompanyConversationCount(
      channelIdOrCompanyId
    );
    ...
    const name =
      request.body.name?.toString() ?? userId + "_" + new Date().getTime();
    const conversationId = admin
      .firestore()
      .collection(SUPPORT_CHANNELS)
      .doc(channelId)
...
export const getCompanyConversationCount = async (
  companyId: string
): Promise<number> => {
