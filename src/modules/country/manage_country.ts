...
export const getSupportedContriesRequest = async (
  request: Request,
  response: Response
) => {
  try {
...
const getLegalDocuments = async (countryCode: string, onlyActive: boolean = true) : Promise<LegalDocument[]|undefined> => {
  try {
    let documentRef = admin.firestore()
    ...
    await Promise.all(documentsData.map(async (doc) => {
      if (doc.storage_link && doc.storage_link.length > 0) {
        ...
        const docUrl = await getPublicLinkForFile(
          doc.storage_link,
          expiration
        );
        doc.url = docUrl;
