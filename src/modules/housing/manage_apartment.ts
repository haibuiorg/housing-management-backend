...
export const addApartmentRequest = async (request: Request, response: Response) => {
  // @ts-ignore
...
export const editApartmentRequest = async (request: Request, response: Response) => {
  // @ts-ignore
  const userId = request.user?.uid;
  const housingCompanyId = request.body.housing_company_id;
...
export const removeUserFromApartmentRequest = async (request: Request, response: Response) => {
  ...
  if (
    !(await isAuthorizedAccessToApartment(userId, housingCompanyId, apartmentId)) &&
...
export const addMultipleApartmentsInBuilding = async (
  ...
  if (!activeSubscription || (activeSubscription.quantity ?? 0) < companyPartmentCount + newApartmentCount) {
    throw {
...
export const addSingleApartment = async (housingCompanyId: string, building: string, houseCode?: string) => {
  const apartmentId = admin
    .firestore()
    .collection(HOUSING_COMPANIES)
    .doc(housingCompanyId)
...
export const getUserApartmentRequest = async (request: Request, response: Response) => {
  // @ts-ignore
  const userId = request.user?.uid;
  const housingCompanyId = request.query.housing_company_id;
  if (!housingCompanyId) {
    response.status(500).send({ errors: { error: 'Missing value', code: 'missing_company_id' } });
  }
  try {
    const apartments = await getUserApartments(userId, housingCompanyId!.toString());
    response.status(200).send(apartments);
  } catch (errors) {
...
export const getSingleApartmentRequest = async (request: Request, response: Response) => {
  // @ts-ignore
  const userId = request.user?.uid;
  const housingCompanyId = request.query.housing_company_id;
  const apartmentId = request.query.apartment_id;
  if (!housingCompanyId || !apartmentId) {
    response.status(500).send({ errors: { error: 'Missing value', code: 'missing_value' } });
  }
  try {
    const apartment = await getUserApartment(userId, housingCompanyId!.toString(), apartmentId!.toString());
    response.status(200).send(apartment);
  } catch (errors) {
    response.status(500).send({ errors: errors });
  }
};
export const getUserApartments = async (userId: string, housingCompanyId: string): Promise<Apartment[]> => {
  if (await isCompanyManager(userId, housingCompanyId)) {
    const apartments = await admin
      .firestore()
      .collectionGroup(APARTMENTS)
...
export const getUserApartment = async (userId: string, housingCompanyId: string, apartmentId: string) => {
  if (await isCompanyManager(userId, housingCompanyId)) {
...
export const addDocumentToApartment = async (request: Request, response: Response) => {
  // @ts-ignore
  const userId = request.user?.uid;
  const companyId = request.body?.housing_company_id;
  const apartmentId = request.body?.apartment_id;
  const isTenant = await isAuthorizedAccessToApartment(userId, companyId, apartmentId);
  if (!isTenant) {
  ...
  if (subscriptionPlan.has_apartment_document !== true) {
    response.status(403).send({ errors: { error: 'Unauthorized', code: 'no_document_access' } });
    return;
  }
...
export const getApartmentDocuments = async (request: Request, response: Response) => {
  // @ts-ignore
  const userId = request.user?.uid;
...
export const deleteApartmentDocuments = async (request: Request, response: Response) => {
  // @ts-ignore
  const userId = request.user?.uid;
  const companyId = request.body?.housing_company_id;
  const apartmentId = request.body?.apartment_id;
  ...
  const basicRef = admin
    ...
    .collection(DOCUMENTS)
    .where(IS_DELETED, '==', false);
  const documents = (await basicRef.orderBy(CREATED_ON, 'desc').get()).docs.map((doc) => doc.data());
  response.status(200).send(documents);
};

export const updateAparmentDocument = async (request: Request, response: Response) => {
  // @ts-ignore
  const userId = request.user?.uid;
  const companyId = request.body?.housing_company_id;
  const apartmentId = request.body?.apartment_id;
...
export const getApartmentDocument = async (request: Request, response: Response) => {
  // @ts-ignore
  const userId = request.user?.uid;
  const companyId = request.params?.housing_company_id;
  const docId = request.params?.document_id;
  const apartmentId = request.params?.apartment_id;
  const isTenant = await isAuthorizedAccessToApartment(userId, companyId?.toString() ?? '', apartmentId);
  if (!isTenant) {
    response.status(403).send({ errors: { error: 'Unauthorized', code: 'not_tenant' } });
    return;
  }
...
export const getApartmentCount = async (companyId: string) => {
  const apartmentCount = (
    await admin.firestore().collection(HOUSING_COMPANIES).doc(companyId).collection(APARTMENTS).count().get()
  ).data().count;
...
export const getApartmentTenant = async (companyId: string, apartmentId: string) => {
  try {
    const apartment = (
      await admin.firestore().collection(HOUSING_COMPANIES).doc(companyId).collection(APARTMENTS).doc(apartmentId).get()
    ).data() as Apartment;
    const combineUsers = [...new Set([...(apartment.owners ?? []), ...(apartment.tenants ?? [])])];
...
export const getApartmentTenantRequest = async (request: Request, response: Response) => {
  ...
  const companyId = request.query?.housing_company_id;
  const apartmentId = request.query?.apartment_id;
  const isTenant = await isApartmentOwner(userId, companyId?.toString() ?? '', apartmentId?.toString() ?? '');
  if (!isTenant && !isAdminRole(userId)) {
    response.status(403).send({ errors: { error: 'Unauthorized', code: 'not_tenant' } });
    return;
  }
  const apartmentTenant = await getApartmentTenant(companyId?.toString() ?? '', apartmentId?.toString() ?? '');
