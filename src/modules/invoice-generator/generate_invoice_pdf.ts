...
const generateHeader = async (doc: PDFKit.PDFDocument, company: Company) => {
  const logo = company.logo_url ? await fetchImage(company.logo_url!) : '';
  doc
      .image(logo, 50, 45, {width: 50, height: 50, fit: [50, 50]})
      .fillColor('#444444')
      .fontSize(20)
...
const generateCustomerInformation =
  (doc: PDFKit.PDFDocument,
      invoice: Invoice, client: User, address: Address) => {
    doc
        .fillColor('#444444')
        .fontSize(20)
        .text('Invoice', 50, 160);
    ...
    doc
        ...
        .font('Helvetica-Bold')
        .text(client.name ?? 'Client', 300, customerInformationTop)
        .font('Helvetica')
        .text(
...
const generateFooter = (
    doc: PDFKit.PDFDocument, bankAccount: BankAccount, invoice: Invoice) => {
  doc
      .fontSize(10)
      .text(
          `Payment is due on: ${new Date(invoice.payment_date)}`,
          50,
          600,
          {align: 'center'},
      )
      ...
      .text(
          'Invoice created with Priorli',
          50,
          700,
          {align: 'center', link: 'www.priorli.com', underline: true},
      ).moveDown();
  codes.loadModules(['code128'], {'includetext': false});
  const logo =codes.create('code128', invoice.virtual_barcode);
  doc
      .image(logo, 50, 750,
          {align: 'center', width: doc.page.width - 100, height: 64});
};
...
const generateTableRow = (
  ...
  doc
      ...
      .text(unitCost, 220, y, {width: 90, align: 'right'})
      .text(quantity, 290, y, {width: 90, align: 'right'})
      .text(taxPercentage, 370, y, {width: 90, align: 'right'} )
