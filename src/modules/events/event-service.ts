...
import { isAdminRole, isAuthorizedAccessToApartment, isCompanyManager } from '../authentication/authentication';
...
export const getEvents = async (request: Request, response: Response) => {
  ...
  let query = admin.firestore().collection(EVENTS).where('start_time', '>=', from).where('start_time', '<=', to);
  ...
  if (!company_id && !apartment_id) {
    ...
    if (apartment_id) {
      ...
      query = query
        ...
        : admin.firestore().collection(EVENTS).where('apartment_id', '==', apartment_id);
    }
  }
  if (types) {
...
const _getEvent = async (userId: string, eventId: string) => {
  try {
    const event = (await admin.firestore().collection(EVENTS).doc(eventId).get()).data() as Event;
    if (
      event.invitees?.includes(userId) ||
      event.type == 'company' ||
      (event.type == 'company_internal' && (await isCompanyManager(userId, event.company_id ?? '')))
    ) {
      return event;
    }
  } catch (errors) {
...
export const createEvents = async (request: Request, response: Response) => {
  ...
  if (type === 'company' || type === 'company_internal') {
    ...
    const apartment = await isAuthorizedAccessToApartment(userId, event.company_id ?? '', event.apartment_id ?? '');
    if (!apartment) {
      response.status(403).send({
        errors: {
          error: 'not_tenant',
...
export const editEvent = async (request: Request, response: Response) => {
  ...
  event.repeat_until = repeat_until == null && repeat == null ? repeat_until : event.repeat_until;
  event.updated_by_name = displayName;
  event.invitees = [...new Set([...(event.invitees ?? []), ...(addition_invitees ?? [])])];
  await admin.firestore().collection(EVENTS).doc(id).update(event);
  response.status(200).send(event);
};

export const removeFromFromEvent = async (request: Request, response: Response) => {
  ...
  event.invitees = event.invitees.filter((item) => !removed_users.includes(item));
  event.updated_by = userId;
  event.updated_by_name = await getUserDisplayName(userId, event.company_id ?? '');
