...
export const updateUserData = async (request: Request, response: Response) => {
  ...
  if (avatarStorageLocation) {
    ...
    // @ts-ignore
    updateField.avatar_url = await getPublicLinkForFile(
      newFileLocation,
      expiration
    );
    // @ts-ignore
...
export const getUserNotificationTokens = async (
  userIds: string[]
): Promise<string[]> => {
  const result: string[] = [];
  await Promise.all(
...
const checkUserEmailVerificationStatus = async (
  request: Request
): Promise<boolean> => {
  // @ts-ignore
...
export const addHousingCompanyToUser = async (
...
export const retrieveUser = async (userId: string): Promise<User|undefined> => {
  ...
  if (
    ...
    const expiration = Date.now() + 604000;
    // @ts-ignore
    const avatarUrl = await getPublicLinkForFile(
      user.avatar_storage_location ?? "",
      expiration
    );
