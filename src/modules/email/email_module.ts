...
export const sendInvitationEmail = async (emails: string[], code: string, companyName: string) => {
  ...
  const msg = {
    ...
    html: invitationEmail(
      ...
      code,
      companyName,
      PRIORLI_PUBLIC_LOGO_URL,
      undefined,
      PRIORLI_INVITATION_EMAIL_SCREENSHOT_LINKS,
    ),
  };
  try {
    sgMail.setApiKey(process.env.SENDGRID ?? '');
...
export const sendManagerAccountCreatedEmail = async (email: string, companyName: string) => {
  try {
    const resetPasswordLink = await admin.auth().generatePasswordResetLink(email);
    const msg = {
      to: email, // Change to your recipient
